...
try:
    ...
    MQTT_DEVICE_ID = _MQTT.get("DEVICE_ID", "<REDACTED_DEVICE_ID>")
    MQTT_USERNAME  = _MQTT.get("USERNAME", "<REDACTED_MQTT_USERNAME>")
    ...
    MQTT_PORT = 1883
    MQTT_DEVICE_ID = "<REDACTED_DEVICE_ID>"
    MQTT_USERNAME = "<REDACTED_MQTT_USERNAME>"
    MQTT_PASSWORD = ""
    ...
    def _rpc_handler_thread(self):
        ...
        while True:
            try:
                ...
                # --- BASIC GET/SET ---
                if mkey in ("rpchelp", "helprpc", "listrpc", "rpcmethods"):
                    response = {
                        "status": "ok",
                        "title": "ESP32 RPC Help",
                        "note": "method name is case-insensitive; use params exactly as documented",
                        "groups": {
                            "status": [
                                {"method": "getStatus", "params": "{}", "desc": "Get motor/usb/led current state"},
                                {"method": "getMotorSpeed", "params": "{}", "desc": "Get fan speed 0-100"},
                                {"method": "getLedBrightness", "params": "{}", "desc": "Get LED brightness 0-100"},
                                {"method": "getUsb1State", "params": "{}", "desc": "Get USB1 state"},
                                {"method": "getUsb2State", "params": "{}", "desc": "Get USB2 state"},
                                {"method": "getLed0", "params": "{}", "desc": "Get RGB LED0 state"},
                                {"method": "getLedMode", "params": "{}", "desc": "Get LED mode auto/manual"}
                            ],
                            "control": [
                                {"method": "setMotorSpeed", "params": "number 0-100", "desc": "Set fan speed"},
                                {"method": "setLedBrightness", "params": "number 0-100", "desc": "Set LED brightness"},
                                {"method": "setLed0", "params": "[r,g,b] | #RRGGBB | {r,g,b}", "desc": "Set LED0 color"},
                                {"method": "setLedMode", "params": "auto | manual", "desc": "Set LED mode"},
                                {"method": "setUsb1", "params": "on/off | 1/0 | true/false", "desc": "Control USB1"},
                                {"method": "setUsb2", "params": "on/off | 1/0 | true/false", "desc": "Control USB2"}
                            ],
                            "profile": [
                                {"method": "listProfiles", "params": "{}", "desc": "List all profiles + active profile"},
                                {"method": "getActiveProfile", "params": "{}", "desc": "Get active profile"},
                                {"method": "setActiveProfile", "params": "profile_name | {name, reset_cycle}", "desc": "Switch profile (optionally reset cycle)"},
                                {"method": "switchProfile", "params": "profile_name | {name, reset_cycle}", "desc": "Alias of setActiveProfile"},
                                {"method": "setProfile", "params": "full plant_profiles.json content", "desc": "Legacy full profile update"},
                                {"method": "updatePlantProfiles", "params": "full plant_profiles.json content", "desc": "Alias of setProfile"}
                            ],
                            "growth": [
                                {"method": "getGrowthState", "params": "{}", "desc": "Get growth state summary"},
                                {"method": "setPlantDate", "params": "{timestamp} | {date_str:'dd/mm/yyyy'|'yyyy-mm-dd'}", "desc": "Set effective plant start date"},
                                {"method": "resetPlantCycle", "params": "{} | {start_date}", "desc": "Reset growth cycle"}
                            ],
                            "profile_update_chunked": [
                                {"method": "startProfileUpdate", "params": "{file_size, sha256}", "desc": "Start chunked update"},
                                {"method": "appendProfileChunk", "params": "{offset, chunk(base64)}", "desc": "Append one chunk"},
                                {"method": "commitProfileUpdate", "params": "{}", "desc": "Validate and apply"},
                                {"method": "abortProfileUpdate", "params": "{}", "desc": "Abort current update"}
                            ]
                        },
                        "examples": [
                            {"method": "rpcHelp", "params": {}},
                            {"method": "getGrowthState", "params": {}},
                            {"method": "setPlantDate", "params": {"date_str": "10/06/2026"}},
                            {"method": "setMotorSpeed", "params": 40},
                            {"method": "setActiveProfile", "params": "petunia_v1"}
                        ]
                    }

                elif mkey == "getmotorspeed":
                ...
                # --- PLANT LIFECYCLE MANAGEMENT ---
                    ...
                    try:
                        ...
                        if isinstance(params, dict):
                            # Ưu tiên timestamp để giữ tương thích và tránh ambiguity
                            timestamp = params.get("timestamp")

                            # Mở rộng: cho phép nhập ngày dễ đọc
                                    ...
                                    if len(ds) > 0:
                                        ...
                                        if ds.find("/") > 0:
                                            ...
                                            if len(parts) == 3:
                                                ...
                                                m = int(parts[1])
                                                y = int(parts[2])
                                                timestamp = time.mktime((y, m, d, 0, 0, 0, 0, 0))
                                        # Format 2: yyyy-mm-dd
                                        elif ds.find("-") > 0:
                                            parts = ds.split("-")
                                            if len(parts) == 3:
                            ...
                            # Giữ behavior cũ: params là số timestamp
                            timestamp = params

                        if not isinstance(timestamp, (int, float)):
                            response = {
                                "status": "error",
                                "message": "timestamp_or_date_str_required",
                                "accepted_date_formats": ["dd/mm/yyyy", "yyyy-mm-dd"]
                            }
                        elif self.local_ai and hasattr(self.local_ai, "growth_state"):
                            ts_i = int(timestamp)
                            self.local_ai.growth_state.set_plant_start_date(ts_i)
                            summary = self.local_ai.growth_state.get_state_summary()
                            response = {
                                "status": "ok",
                                "message": "plant_date_updated",
                                "resolved_timestamp": ts_i,
                                "input": input_payload,
                                "state": summary
                            }
                        else:
