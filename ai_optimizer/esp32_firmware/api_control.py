...
# OWM (callapi.py) + Open-Meteo (om_* keys) + AQICN (aqi_* keys)
# Safe if callapi.py is missing. Token configured in config.py / AQICN_TOKEN_HARDCODED.
...
AQICN_TOKEN_HARDCODED = ""  # Placeholder — set AQICN_TOKEN in config.py
...
class APIControl:
    ...
    Unified weather provider:
      - OWM via callapi.get_weather()  (original flat keys)
      - Open-Meteo snapshot           (om_* keys)
      - AQICN air quality             (aqi_* keys)
    get_weather(timeout) -> dict
    """
    def __init__(self, latitude=None, longitude=None, timezone=None,
                 enable_open_meteo=None, enable_aqicn=None,
                 cache_ttl_sec=None, http_timeout_sec=None,
                 aqicn_token=""):
        ...
        _token_from_cfg = getattr(CFG, "AQICN_TOKEN", "") if CFG else ""
        self._aqicn_token = aqicn_token or _token_from_cfg or AQICN_TOKEN_HARDCODED
    ...
    def _get_owm_cached(self, ttl_sec, timeout):
        ...
        if _callapi_get_weather is None:
            c["err"] = "callapi.get_weather() not found"; c["ts"] = _now_ms(); c["data"] = None
            return None
        try:
            data = _callapi_get_weather(timeout=timeout)
            if not isinstance(data, dict):
                raise ValueError("non-dict")
            c["data"] = data; c["err"] = None
        except Exception as e:
            c["data"] = None; c["err"] = str(e)
        c["ts"] = _now_ms()
        return c["data"]
    ...
    def _get_open_meteo_cached(self, ttl_sec, timeout):
        ...
        if _is_fresh(c["ts"], ttl_sec * 1000) and c["data"] is not None:
            return c["data"]
        try:
            c["data"] = self._fetch_open_meteo(self.lat, self.lon, self.tz, timeout)
            c["err"] = None
        except Exception as e:
            c["data"] = None; c["err"] = str(e)
        c["ts"] = _now_ms()
    ...
    def _fetch_open_meteo(self, lat, lon, timezone, timeout):
        ...
        params = (
            ...
            "cloud_cover,wind_speed_10m,wind_direction_10m,wind_speed_180m,"
            "wind_direction_180m,temperature_180m,soil_temperature_54cm,"
        ...
        return {
            "om_next_temp_c":               t2m[0]   if t2m   else None,
            "om_next_rh_pct":               rh2m[0]  if rh2m  else None,
            "om_next_dewpoint_c":           dp2m[0]  if dp2m  else None,
            "om_next_precip_mm":            prec[0]  if prec  else None,
            "om_next_cloudcover_pct":       cc[0]    if cc    else None,
            "om_next_windspeed_ms":         ws10[0]  if ws10  else None,
            "om_next_winddir_deg":          wd10[0]  if wd10  else None,
            "om_next_windspeed_180m_ms":    ws180[0] if ws180 else None,
            "om_next_winddir_180m_deg":     wd180[0] if wd180 else None,
            "om_next_temp_180m_c":          t180[0]  if t180  else None,
            "om_next_soil_temp_54cm_c":     st54[0]  if st54  else None,
            "om_next_soil_moisture_27_81cm_m3m3": sm2781[0] if sm2781 else None,
            ...
            "om_lat": lat, "om_lon": lon, "om_timezone": j.get("timezone"),
        }
    ...
    def _get_aqicn_cached(self, ttl_sec, timeout):
        ...
        if _is_fresh(c["ts"], ttl_sec * 1000) and c["data"] is not None:
            return c["data"]
        try:
            c["data"] = self._fetch_aqicn(self.lat, self.lon, self._aqicn_token, timeout)
            c["err"] = None
        except Exception as e:
            c["data"] = None; c["err"] = str(e)
        c["ts"] = _now_ms()
